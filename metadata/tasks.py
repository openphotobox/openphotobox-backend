...
from .models import AssetMetadata, KeywordTag, AssetKeyword, ClipEmbedding
...
def process_asset_metadata(self, asset_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Rewind buffer in case previous read advanced it
            image_data.seek(0)
            img_w, img_h = _get_image_dimensions(image_data, exif_data)
            if img_w and img_h:
        ...
        try:
            ...
            if taken_dt is not None:
                ...
                if taken_dt.tzinfo is None:
                    ...
                    default_tz_name = getattr(settings, 'OPENPHOTOBOX', {}).get('DEFAULT_CAPTURE_TZ', 'America/New_York')
        ...
        metadata, created = AssetMetadata.objects.get_or_create(
            ...
            defaults={
                ...
                'flash_fired': _parse_flash_fired(exif_data.get('Flash')),
        ...
        if not created:
            ...
            metadata.flash_fired = _parse_flash_fired(exif_data.get('Flash'))
    ...
    try:
        ...
        if hasattr(image, '_getexif') and image._getexif() is not None:
            ...
            for tag_id, value in exif.items():
                ...
                if tag in ('XPComment', 'XPSubject', 'XPTitle', 'XPKeywords', 'XPAuthor') and isinstance(value, (bytes, bytearray)):
                    try:
                    ...
                    # GPSInfo may contain nested non-text bytes/tuples; convert safely
                    if tag == 'GPSInfo':
                        try:
                            # Convert mapping to JSON-safe types
                            gps = {}
                            if isinstance(value, dict):
                                for k, v in value.items():
                                    gps[str(k)] = _sanitize_for_json(_convert_pil_value(v))
                            else:
                                gps = _sanitize_for_json(_convert_pil_value(value))
                            exif_data[tag] = gps
                        except Exception:
                            exif_data[tag] = _sanitize_for_json(_convert_pil_value(value))
                    else:
...
def _parse_flash_fired(flash_value: Optional[object]) -> Optional[bool]:
    """Parse EXIF Flash tag to a boolean indicating if the flash fired.

    The EXIF Flash tag is a bit-field integer. Bit 0 indicates whether a flash fired.
    Values are often provided as ints, floats, or strings by different libraries.
    Returns True/False, or None if value is missing or cannot be parsed.
    """
    if flash_value is None:
        return None
    try:
        if isinstance(flash_value, bool):
            return flash_value
        if isinstance(flash_value, str):
            # Normalize string to numeric if possible
            flash_value = float(flash_value) if ('.' in flash_value) else int(flash_value)
        if isinstance(flash_value, float):
            flash_value = int(flash_value)
        # At this point assume integer bit-field
        return bool(int(flash_value) & 0x1)
    except Exception:
        return None
...
def _generate_keywords_from_exif(asset: Asset, exif_data: Dict[str, Any]) -> None:
    ...
    for keyword_name in keywords_to_create:
        keyword, created = KeywordTag.objects.get_or_create(
            name=keyword_name,
            defaults={'slug': keyword_name.replace(' ', '-')}
        )
        ...
        AssetKeyword.objects.get_or_create(
            asset=asset,
            keyword=keyword,
            defaults={
                'source': 'exif',
