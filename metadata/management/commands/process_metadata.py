...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--asset-id',
            type=str,
            help='Process metadata for a specific asset ID'
        )
        parser.add_argument(
            '--all',
            action='store_true',
            help='Process metadata for all assets'
        )
        parser.add_argument(
            '--limit',
            type=int,
    ...
    def process_asset(self, asset, use_async=False):
        ...
        if use_async:
            ...
            try:
                ...
                if result['success']:
                    self.stdout.write(
                        self.style.SUCCESS(f'  ✓ Processed successfully')
                    )
                    if result.get('metadata_created'):
                        self.stdout.write(f'    Created new metadata record')
                    ...
                    if result.get('description_extracted'):
                        self.stdout.write(f'    Extracted description from metadata')
                else:
                    self.stdout.write(
