...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--limit", type=int, default=50, help="Limit number of assets to process (default: 50)")
        parser.add_argument("--offset", type=int, default=0, help="Offset for pagination (default: 0)")
        parser.add_argument("--all", action="store_true", help="Process all assets (ignores limit)")
    ...
    def handle(self, *args, **options):
        ...
        for i, asset in enumerate(assets):
            try:
                ...
                self.stdout.write(self.style.ERROR(f"  [{i+1:3d}] Failed to queue asset {asset.id}: {e}"))

        self.stdout.write(f'\n{self.style.SUCCESS("✓")} Queued {len(queued_tasks)} tasks for async processing')
        self.stdout.write("Monitor progress with: celery -A openphotobox_backend inspect active")
        self.stdout.write("Check results with: celery -A openphotobox_backend inspect stats")
