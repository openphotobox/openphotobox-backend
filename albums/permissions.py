...
def get_accessible_albums(user) -> QuerySet:
    ...
    return Album.objects.filter(Q(owner=user) | Q(shares__shared_with=user)).distinct()
...
def can_view_asset(user, asset) -> bool:
    ...
    return AlbumAsset.objects.filter(asset=asset, album__in=accessible_albums).exists()
...
def filter_accessible_asset_ids(user, asset_ids):
    ...
    return set(accessible_assets.filter(id__in=asset_ids).values_list("id", flat=True))
