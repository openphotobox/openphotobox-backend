...
import numpy as np
from .models import Person, Face, PersonMergeSuggestion
...
from .tasks import _calculate_centroid
from .tasks import create_fresh_face_app
from .serializers import (
    PersonSerializer, FaceSerializer, PersonMergeSuggestionSerializer,
    PersonMergeRequestSerializer, FaceAssignmentSerializer, FaceUnassignmentSerializer,
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def merge(self, request, pk=None):
        ...
        if source_ids:
            ...
            return Response({'detail': 'Provide source_person_ids or target_person_id'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        with transaction.atomic():
            ...
            def is_generic(name: str):
                return not name or name.strip().lower() in { 'person', 'unknown' } or name.strip().lower().startswith('person ')
...
class FaceViewSet(viewsets.ModelViewSet):
    ...
    def unassign(self, request):
        ...
        with transaction.atomic():
            ...
            affected_person_ids = set(
                Face.objects.filter(id__in=face_ids, person__isnull=False)
    ...
    def manual_create(self, request):
        ...
        try:
            import requests, numpy as _np, cv2 as _cv2
            ...
            if image is not None:
                ...
                x2 = min(W, int((x + w) * W)); y2 = min(H, int((y + h) * H))
                if x2 > x1 and y2 > y1:
    ...
    def candidates(self, request, pk=None):
        ...
        if face_emb.size == 0 and getattr(face, 'detection_model', '') == 'manual':
            try:
                ...
                if image is not None:
                    H, W = image.shape[:2]
                    x1 = max(0, int(face.x * W)); y1 = max(0, int(face.y * H))
                    x2 = min(W, int((face.x + face.w) * W)); y2 = min(H, int((face.y + face.h) * H))
                    if x2 > x1 and y2 > y1:
                        crop = image[y1:y2, x1:x2]
                        app = create_fresh_face_app()
                        dets = app.get(crop)
            ...
            if face_emb.size > 0:
                for proto in proto_qs:
                    ...
                    if proto_emb.size == face_emb.size:
                        sim = float(np.dot(face_emb, proto_emb) / (np.linalg.norm(face_emb) * np.linalg.norm(proto_emb)))
                        if sim > best_sim:
                            best_sim = sim
...
class PersonMergeSuggestionViewSet(viewsets.ModelViewSet):
    ...
    queryset = PersonMergeSuggestion.objects.select_related(
        'person_a', 'person_b', 'reviewed_by'
    ).all()
