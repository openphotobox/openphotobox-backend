...
class PersonSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Person
        fields = [
            'id', 'display_name', 'aka', 'notes', 'face_count', 'asset_count',
            'headshot_url', 'embedding_centroid', 'created_at', 'updated_at'
        ]
...
class FaceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Face
        fields = [
            'id', 'asset', 'asset_id', 'person', 'person_name',
            'x', 'y', 'w', 'h', 'embedding', 'quality', 'detection_confidence',
            'thumbnail_url', 'person_headshot_url',
            'detection_model', 'created_at', 'updated_at'
        ]
...
class PersonMergeSuggestionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'cosine_similarity', 'confidence_score', 'status', 'reviewed_by', 'reviewed_at',
            'created_at'
        ]
...
class PersonMergeRequestSerializer(serializers.Serializer):
    ...
    source_person_ids = serializers.ListField(
        ...
        min_length=1,
        help_text="List of person IDs to merge into the target person"
    )
    delete_source_persons = serializers.BooleanField(
        default=True,
        help_text="Whether to delete source persons after merging"
    )
...
class FaceAssignmentSerializer(serializers.Serializer):
    ...
    face_ids = serializers.ListField(
        child=serializers.UUIDField(),
        min_length=1,
        help_text="List of face IDs to assign"
    )
    person_id = serializers.UUIDField(
        help_text="Person ID to assign faces to"
    )
...
class FaceUnassignmentSerializer(serializers.Serializer):
    ...
    face_ids = serializers.ListField(
        child=serializers.UUIDField(),
        min_length=1,
        help_text="List of face IDs to unassign (set person to null)"
    )
