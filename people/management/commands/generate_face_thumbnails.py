...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--limit", type=int, default=200, help="Limit the number of faces to process (default: 200)"
        )
        parser.add_argument("--async", action="store_true", help="Process asynchronously using Celery workers")

    def handle(self, *args, **options):
        qs = Face.objects.filter(Q(thumbnail__isnull=True) | Q(thumbnail__is_ready=False)).order_by(
