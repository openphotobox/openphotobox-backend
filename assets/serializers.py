...
from .models import Asset, Album, UploadBatch, StorageBackend, StorageBucket
...
class StorageBackendSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'name', 'backend_type', 'endpoint_url', 'region',
...
class StorageBucketSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StorageBucket
        fields = [
            'id', 'backend', 'backend_name', 'backend_type', 'name', 
            'display_name', 'purpose', 'is_public', 'path_prefix',
...
class AssetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Asset
        fields = [
            'id', 'sha256', 'storage_bucket', 'storage_bucket_name', 
            'storage_backend_name', 'storage_key', 'storage_url', 'storage_path',
            'mime_type', 'width', 'height', 'taken_at',
            'description', 'keyword_names', 'phash',
            'thumbnail_url', 'thumbnail_urls', 'original_url',
            'metadata', 'faces', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'sha256', 'storage_url', 'storage_path', 'keyword_names', 'thumbnail_url', 'thumbnail_urls', 'original_url', 'metadata', 'faces', 'created_at', 'updated_at']
    ...
    def get_thumbnail_urls(self, obj):
        ...
        return {
            thumbnail.size: thumbnail.storage_url
...
class AssetGallerySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Asset
        fields = [
            'id', 'width', 'height', 'taken_at', 'created_at',
            'thumbnail_url', 'original_url', 'storage_url', 'mime_type', 'description'
        ]
...
class AlbumSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Album
        fields = [
            'id', 'title', 'description', 'cover_asset',
...
class UploadBatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UploadBatch
        fields = [
            'id', 'name', 'total_files', 'processed_files', 'failed_files',
            'progress_percentage', 'default_keywords', 'default_album',
            'created_by', 'created_at', 'updated_at', 'completed_at'
        ]
        read_only_fields = [
            'id', 'processed_files', 'failed_files', 'created_by',
