...
def generate_asset_thumbnails(self, asset_id: str, sizes: list = None) -> Dict[str, Any]:
    """
    Generate thumbnails for an asset at different sizes.
    
    Args:
        asset_id: UUID of the asset to process
        sizes: List of thumbnail sizes to generate (default: all sizes)
    ...
    try:
        ...
        if face_thumbnail:
            ...
            return {
                'success': True,
                'face_id': face_id,
                'thumbnail_id': str(face_thumbnail.id)
            }
        else:
...
def _download_asset_image(asset: Asset) -> Optional[BytesIO]:
    ...
    try:
        # Create upload service to download from storage
...
def _get_thumbnails_bucket() -> StorageBucket:
    ...
    try:
        ...
        logger.info(f"No thumbnails bucket found, using originals bucket: {originals_bucket.name} with thumbnails/ prefix")
        return originals_bucket


def _generate_single_thumbnail(asset: Asset, original_image: Image.Image, size: str, bucket: StorageBucket) -> Optional[AssetThumbnail]:
