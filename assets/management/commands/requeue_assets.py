...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--recent", type=int, help="Only process assets uploaded in the last N days")
        parser.add_argument("--no-metadata", action="store_true", help="Only process assets that have no metadata")
        parser.add_argument(
            "--missing-dimensions", action="store_true", help="Only process assets where width/height are zero"
        )
        parser.add_argument("--no-faces", action="store_true", help="Only process assets that have no face detection")
        parser.add_argument("--mime-type", help="Only process assets with specific MIME type (e.g., image/jpeg)")
        parser.add_argument("--asset-ids", nargs="+", help="Process specific asset IDs")
    ...
    def handle(self, *args, **options):
        ...
        if metadata_only and faces_only:
            self.stdout.write(self.style.ERROR("Cannot specify both --metadata-only and --faces-only"))
            return
        ...
        if total_assets == 0:
            self.stdout.write(self.style.WARNING("No assets found matching the criteria"))
            return
        ...
        self.stdout.write("")
        if errors > 0:
            self.stdout.write(self.style.WARNING(f"⚠️  Completed with {errors} errors"))
