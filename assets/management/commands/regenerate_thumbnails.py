...
from assets.models import Asset, AssetThumbnail
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force regeneration even if thumbnails already exist'
        )
        parser.add_argument(
            '--missing-only',
            action='store_true',
            help='Only generate thumbnails for assets that have no thumbnails'
        )
        parser.add_argument(
            '--asset-ids',
            nargs='+',
            help='Specific asset IDs to process (UUIDs)'
        )
    ...
    def handle(self, *args, **options):
        ...
        if asset_ids:
            ...
            if not assets.exists():
                self.stdout.write(
                    self.style.ERROR(f'No assets found with IDs: {asset_ids}')
                )
                return
        else:
            # Get all assets
        ...
        if total_assets == 0:
            self.stdout.write(
                self.style.WARNING('No assets found to process')
            )
            return
